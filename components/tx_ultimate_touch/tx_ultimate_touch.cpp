...
void TxUltimateTouch::setup() {
  ESP_LOGI(TAG, "Tx Ultimate Touch initialized (long_press_x_offset=%d)", this->long_press_x_offset_);
}
...
void TxUltimateTouch::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Long Press X Offset: %d", this->long_press_x_offset_);
}

void TxUltimateTouch::send_touch_(TouchPoint tp) {
  switch (tp.state) {
    case TOUCH_STATE_RELEASE:
      if (tp.x >= this->long_press_x_offset_) {
        tp.x = tp.x - this->long_press_x_offset_;
